...
module.exports = async (client) => {
  (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async cmdFile => {
    ...
    if (!cmd.name) return Logger.warn(`Commande non-Chargée: ajouter un nom à la commande ↓\nFichier -> ${cmdFile}`);

    if (!cmd.description && cmd.type != 'USER') return Logger.warn(`Commande non-Chargée: ajouter une description à la commande ↓\nFichier -> ${cmdFile}`);

    if (!cmd.category) return Logger.warn();(`Commande non-Chargée: ajouter une categorie à la commande ↓\nFichier -> ${cmdFile}`);

    if (!cmd.permissions) return Logger.warn(`Commande non-Chargée: ajouter des permissions à la commande ↓\nFichier -> ${cmdFile}`);

    if (cmd.ownerOnly == undefined) return Logger.warn(`Commande non-Chargée: indiquer si la commande est ownerOnly ↓\nFichier -> ${cmdFile}`);

    if (!cmd.usage) return Logger.warn(`Commande non-Chargée: ajouter une utilisation (usage) à la commande ↓\nFichier -> ${cmdFile}`);

    if (!cmd.examples) return Logger.warn(`Commande non-Chargée: ajouter des exemples (examples) à la commande ↓\nFichier -> ${cmdFile}`);
